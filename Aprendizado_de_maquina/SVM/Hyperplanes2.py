...
variables = ['GenNCSBearingTemp', 'GenSpeed', 'GridCurrent', 'GenActivePower', 'RotorSpeed']          # variaveis analogicas a serem utilizadas
...
cur.execute("SELECT dbo.GenNCSBearingTemp_A20.TimeStampDateTimeUTC, dbo.GenNCSBearingTemp_A20.FieldValueConvert, "
            "dbo.GenSpeed_A20.FieldValueConvert, dbo.GridCurrent_A20.FieldValueConvert, dbo.GenActivePower_A20.FieldValueConvert, "
            "dbo.RotorSpeed_A20.FieldValueConvert "
            ...
            "dbo.GridCurrent_A20 ON dbo.GenSpeed_A20.TimeStampDateTimeUTC = dbo.GridCurrent_A20.TimeStampDateTimeUTC INNER JOIN "
            "dbo.GenActivePower_A20 ON dbo.GridCurrent_A20.TimeStampDateTimeUTC = dbo.GenActivePower_A20.TimeStampDateTimeUTC INNER JOIN "
            "dbo.RotorSpeed_A20 ON dbo.GenActivePower_A20.TimeStampDateTimeUTC = dbo.RotorSpeed_A20.TimeStampDateTimeUTC "
...
for row in cur:
    ...
    val_genAc20.append(row[4])
    val_rotS20.append(row[5])
...
val_genAc20 = np.array(val_genAc20)
val_rotS20 = np.array(val_rotS20)
...
cur.execute("SELECT dbo.GenNCSBearingTemp_A58.TimeStampDateTimeUTC, dbo.GenNCSBearingTemp_A58.FieldValueConvert, "
            "dbo.GenSpeed_A58.FieldValueConvert, dbo.GridCurrent_A58.FieldValueConvert, dbo.GenActivePower_A58.FieldValueConvert, "
            "dbo.RotorSpeed_A58.FieldValueConvert "
            ...
            "dbo.GridCurrent_A58 ON dbo.GenSpeed_A58.TimeStampDateTimeUTC = dbo.GridCurrent_A58.TimeStampDateTimeUTC INNER JOIN "
            "dbo.GenActivePower_A58 ON dbo.GridCurrent_A58.TimeStampDateTimeUTC = dbo.GenActivePower_A58.TimeStampDateTimeUTC INNER JOIN "
            "dbo.RotorSpeed_A58 ON dbo.GenActivePower_A58.TimeStampDateTimeUTC = dbo.RotorSpeed_A58.TimeStampDateTimeUTC "
            "ORDER BY [TimeStampDateTimeUTC]; ")
...
# -------------------------------------------------------------------------------------------------------------------- #
dados1 = {'Timestamp': tsSemOut20, 'GenNCSBearingTemp': val1SemOut20, 'GenSpeed': val2SemOut20, 'GridCurrent': val3SemOut20, 'GenActivePower': val4SemOut20, 'RotorSpeed': val5SemOut20}
df_analogic1 = pd.DataFrame(data=dados1)
dados2 = {'Timestamp': tsSemOut58, 'GenNCSBearingTemp': val1SemOut58, 'GenSpeed': val2SemOut58, 'GridCurrent': val3SemOut58, 'GenActivePower': val4SemOut58, 'RotorSpeed': val5SemOut58}
...
scatter_matrix(xtrain, alpha=0.2, figsize=(10.8, 7.2), diagonal='kde')
...
# abdtclassifier = AdaBoostClassifier(DecisionTreeClassifier(max_depth=1),
#                         algorithm="SAMME",
#                         n_estimators=200)
# gpclassifier = GaussianProcessClassifier(1.0 * RBF(1.0))
# kncclassifier = KNeighborsClassifier()

t0 = time()
svclassifier.fit(xtrain, ytrain)
# rfclassifier.fit(xtrain, ytrain)
# abdtclassifier.fit(xtrain, ytrain)
# kncclassifier.fit(xtrain, ytrain)

print("tempo de treinamento:", round(time()-t0, 3), "s")
t1 = time()
y_pred = svclassifier.predict(xtest)
# y_pred = rfclassifier.predict(xtest)
# y_pred = abdtclassifier.predict(xtest)
# y_pred = knccclassifier.predict(xtest)
...
feature_list = list(xtest.columns)
