...
cur.execute("SELECT dbo.GenNCSBearingTemp_A20.TimeStampDateTimeUTC, dbo.GenNCSBearingTemp_A20.FieldValueConvert, "
            "dbo.GenSpeed_A20.FieldValueConvert, dbo.StatorCurrent_A20.FieldValueConvert "
            ...
            "dbo.StatorCurrent_A20 ON dbo.GenSpeed_A20.TimeStampDateTimeUTC = dbo.StatorCurrent_A20.TimeStampDateTimeUTC "
...
'''val_genNCS20 = np.array(errorsV120)
val_genS20 = np.array(errorsV220)
val_curGrid20 = np.array(errorsV320)
timestamp20 = np.array(errorsTS20)'''
...
cur.execute("SELECT dbo.GenNCSBearingTemp_A58.TimeStampDateTimeUTC, dbo.GenNCSBearingTemp_A58.FieldValueConvert, "
            "dbo.GenSpeed_A58.FieldValueConvert, dbo.StatorCurrent_A58.FieldValueConvert "
            ...
            "dbo.StatorCurrent_A58 ON dbo.GenSpeed_A58.TimeStampDateTimeUTC = dbo.StatorCurrent_A58.TimeStampDateTimeUTC "
            "ORDER BY [TimeStampDateTimeUTC]; ")
...
'''val_genNCS58 = np.array(errorsV158)
val_genS58 = np.array(errorsV258)
val_curGrid58 = np.array(errorsV358)
timestamp58 = np.array(errorsTS58)'''
...
# -------------------------------------------------------------------------------------------------------------------- #
dados1 = {'Timestamp': tsSemOut20, 'GenNCSBearingTemp': val1SemOut20, 'GenSpeed': val2SemOut20, 'StatorCurrent': val3SemOut20}
df_analogic1 = pd.DataFrame(data=dados1)
dados2 = {'Timestamp': tsSemOut58, 'GenNCSBearingTemp': val1SemOut58, 'GenSpeed': val2SemOut58, 'StatorCurrent': val3SemOut58}
...
for n in al['TimeIniUTC'][al['AlarmAero'] == 'MOR05']:
    df_analogic2.loc[np.where(abs(n - df_analogic2.Timestamp) < timedelta(0, 7200))[0], 'Labels'] = 1

for n in al['TimeIniUTC'][al['AlarmAero'] == 'CF206']:
    df_analogic1.loc[np.where(abs(n - df_analogic1.Timestamp) < timedelta(0, 7200))[0], 'Labels'] = 1
...
first_cut_date1 = np.argwhere(abs(df_analogic1['Timestamp'] - pd.Timestamp(2017, 4, 19)) < timedelta(0, 600))[0][0]
second_cut_date1 = np.argwhere(abs(df_analogic1['Timestamp'] - pd.Timestamp(2017, 5, 10)) < timedelta(0, 600))[0][0]
third_cut_date1 = np.argwhere(abs(df_analogic1['Timestamp'] - pd.Timestamp(2017, 9, 21)) < timedelta(0, 600))[0][0]
fourth_cut_date1 = np.argwhere(abs(df_analogic1['Timestamp'] - pd.Timestamp(2017, 10, 12)) < timedelta(0, 600))[0][0]
fifith_cut_date1 = np.argwhere(abs(df_analogic1['Timestamp'] - pd.Timestamp(2018, 1, 2)) < timedelta(0, 600))[0][0]
sixth_cut_date1 = np.argwhere(abs(df_analogic1['Timestamp'] - pd.Timestamp(2018, 1, 7)) < timedelta(0, 600))[0][0]
seventh_cut_date1 = np.argwhere(abs(df_analogic1['Timestamp'] - pd.Timestamp(2018, 1, 9, 10)) < timedelta(0, 600))[0][0]

first_cut_date2 = np.argwhere(abs(df_analogic2['Timestamp'] - pd.Timestamp(2017, 4, 19)) < timedelta(0, 600))[0][0]
second_cut_date2 = np.argwhere(abs(df_analogic2['Timestamp'] - pd.Timestamp(2017, 5, 10)) < timedelta(0, 600))[0][0]
third_cut_date2 = np.argwhere(abs(df_analogic2['Timestamp'] - pd.Timestamp(2017, 9, 21)) < timedelta(0, 600))[0][0]
fourth_cut_date2 = np.argwhere(abs(df_analogic2['Timestamp'] - pd.Timestamp(2017, 10, 12)) < timedelta(0, 600))[0][0]
fifith_cut_date2 = np.argwhere(abs(df_analogic2['Timestamp'] - pd.Timestamp(2018, 1, 2)) < timedelta(0, 600))[0][0]
sixth_cut_date2 = np.argwhere(abs(df_analogic2['Timestamp'] - pd.Timestamp(2018, 1, 7)) < timedelta(0, 600))[0][0]
seventh_cut_date2 = np.argwhere(abs(df_analogic2['Timestamp'] - pd.Timestamp(2018, 1, 9, 10)) < timedelta(0, 600))[0][0]
...
result_train.drop('GenSpeed', axis=1, inplace=True)
...
result_test.drop('GenSpeed', axis=1, inplace=True)
...
#clf = neighbors.KNeighborsClassifier(n_neighbors=8, weights='uniform', p=2)
